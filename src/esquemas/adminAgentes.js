...
const esquemaCrearRegistradorAgente = z.object({
  ...
  unitId: z.coerce.number().int().min(0).max(255).optional(),
  indiceInicial: z.coerce.number().int().min(0).max(65535),
  cantidadRegistros: z.coerce.number().int().min(1).max(125),
  intervaloSegundos: z.coerce.number().int().min(1).max(3600).optional(),
...
const esquemaActualizarRegistradorAgente = z.object({
  ...
  unitId: z.coerce.number().int().min(0).max(255).optional(),
  indiceInicial: z.coerce.number().int().min(0).max(65535).optional(),
  cantidadRegistros: z.coerce.number().int().min(1).max(125).optional(),
  intervaloSegundos: z.coerce.number().int().min(1).max(3600).optional(),
  activo: z.boolean().optional(),
  alimentadorId: z.string().uuid().nullable().optional(),
  tipoDispositivo: z.string().trim().max(50).optional(),
